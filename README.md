<img align="right" src="https://visitor-badge.laobi.icu/badge?page_id=twahidulislamdev.twahidulislamdev"/>

<br>

<!-- Elegant Hero with Refined Typing SVG -->
<div align="center">
  <img
    src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=36&duration=4200&pause=1000&color=ff6e96&center=true&vCenter=true&width=620&height=90&lines=Hi+There!+✨;I'm+Twahidul+Islam;Full-Stack+MERN+Developer;Based+In+Dhaka%2C+Bangladesh"
    alt="Twahidul Islam - Bold Typing Animation"
  />
</div>

<!-- Tagline -->
<div align="center" style="padding: 0 10px;">
  <hr style="border: none; height: 3px; background: linear-gradient(to right, #a78bfa, #c084fc, #a78bfa); border-radius: 3px; opacity: 0.7; margin: 28px 0;" />
  
  <h3 align="center">
    Full-Stack Developer | Problem Solver | Web Builder
  </h3>
  
  <p>
    🔭 I build web applications that people actually enjoy using.<br>
    My sweet spot? Taking complex problems and turning them into clean, fast, scalable solutions.<br>
    I care deeply about writing code that's not just functional, but thoughtful — code that other developers can work with and users can rely on.<br>
    For me, every project is a chance to learn something new and build something I'm genuinely proud of.
  </p>
  
  <p>
    ✨📝 Fun fact: My commit messages tell better stories than my diary ever did. 📝✨
  </p>
  
  <hr style="border: none; height: 3px; background: linear-gradient(to right, #a78bfa, #c084fc, #a78bfa); border-radius: 3px; opacity: 0.7; margin: 28px 0;" />
</div>

<!-- Social Links -->
<div align="center">
  <h3>Where to Find Me</h3>
  <a href="https://www.linkedin.com/in/twahidulislamdev-july2005">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="https://twitter.com/twahidulislam">
    <img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" alt="Twitter"/>
  </a>
  <a href="https://fb.com/twahidulislamdev">
    <img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook"/>
  </a>
  <a href="https://instagram.com/twahidulislamdev">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"/>
  </a>
  <a href="https://t.me/Twahidulislam">
    <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram"/>
  </a>
  <a href="mailto:twahid746@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/>
  </a>
</div>

<!-- Current Focus -->
<div align="center">
  <h3 align="center">What I'm Working On Right Now</h3>
  <table align="center">
    <tr>
      <td>
        <ul>
          <li>Deep diving into backend with Express.js and MongoDB</li>
          <li>Building full-stack applications with Next.js</li>
          <li>Exploring mobile development with React Native</li>
          <li>Mastering API architecture and database design</li>
        </ul>
      </td>
    </tr>
  </table>
</div>

<!-- Tech Stack -->
<h3 align="center">Tech Stack</h3>
<table align="center" cellpadding="12">
  <tr>
    <td align="center">
      <b>Frontend</b><br>
      <img src="https://skillicons.dev/icons?i=html,css,tailwind,bootstrap,react,redux,nextjs" alt="Frontend"/>
    </td>
    <td align="center">
      <b>Backend & Data</b><br>
      <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,firebase" alt="Backend"/>
    </td>
    <td align="center">
      <b>Mobile App</b><br>
      <img src="https://skillicons.dev/icons?i=reactnative" alt="React Native"/>
    </td>
    <td align="center">
      <b>Tools</b><br>
      <img src="https://skillicons.dev/icons?i=github,vscode,git,figma,npm" alt="Tools"/>
    </td>
  </tr>
</table>

<!-- GitHub Activity - FIXED SIDE BY SIDE -->
<h3 align="center">GitHub Activity</h3>
<div align="center" style="display: flex; justify-between: center; align-items: center; gap: 10px; ;">
  <img src="https://streak-stats.demolab.com?user=twahidulislamdev&theme=dracula&hide_border=true&border_radius=10"
       width="370" height="195"
       alt="GitHub Contribution Streak"/>
  
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=twahidulislamdev&layout=compact&theme=dracula&hide_border=true&border_radius=10&langs_count=4&cache_seconds=1800"
       width="370" height="195"
       alt="Top Languages"/>
      
</div>

<!-- Personality Note -->
<div align="center">
  <h3>A Note</h3>
  <blockquote>
    <p>Driven by clean architecture and continuous learning.<br>
    Balancing deep-focus coding sessions with strategic downtime.</p>
  </blockquote>
</div>
